...
void calculatePredict(std::vector<double> s, double v, double yaw_rate, double dt) {
	double x, y, theta;

	x = s[0] + v / yaw_rate * (sin(s[2] + yaw_rate * dt) - sin(s[2]));
	y = s[1] + v / yaw_rate * (cos(s[2]) - cos(s[2] + yaw_rate * dt));
	theta = s[2] + yaw_rate * dt;
...
int main() {
	...
	s.push_back(102.0);
	s.push_back(65);
	s.push_back(5*M_PI/8);

	// s << 102.0, 65.0, 5*M_PI/8;

	calculatePredict(s, 110, M_PI/8, 0.1);
